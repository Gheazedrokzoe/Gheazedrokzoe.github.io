...
非常高妙的题目。注意到所有点的答案其实是关于原点对称的。所以考虑直接求出所有点的答案。每一次我们可以把跨过原点的较短的一边的答案直接归并到另一侧，删去那些点。这样做每一个点最多被删一次。

太牛了。

### [ARC146C] Even XOR
考虑已经有了一个大小为 $S$ 的集合怎么得到大小为 $S+1$ 的集合。然后是两个结论:

- 一个集合内所有的长度为奇数的子集异或和互不相同。
- 所以你加入的新的一个数也和上面的所有和互不相同。

所以你每次就是只能加入 $2^n - 2^{S-1}$ 种不同的数。

注意到是无序，所以你要除以一个 $1\over{S+1}$。直接递推即可。

### [ARC139C] One Three Nine

https://www.luogu.com.cn/article/jum0fwen
